...
def get_frame(vidcap: cv.VideoCapture, msec: float):
    """
    """
...
def display_image(image: np.ndarray, points: Optional[np.ndarray] = None,
                  image_time: float = 0):
...
def click_tracking_points(image: np.ndarray) -> np.ndarray:
    """
    """
    image = image.copy()
    points = []
...
def track_points_seq(images: np.ndarray):
    """
    """
    initial_points = click_tracking_points(images[0])
